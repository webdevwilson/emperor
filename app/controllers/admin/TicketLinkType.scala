...
object TicketLinkType extends Controller with Secured {
  ...
  def edit(ltypeId: Long) = IsAuthorized(0, "PERM_GLOBAL_ADMIN") { implicit request =>
  ...
  def item(ltypeId: Long) = IsAuthorized(0, "PERM_GLOBAL_ADMIN") { implicit request =>
  ...
  def update(ltypeId: Long) = IsAuthorized(0, "PERM_GLOBAL_ADMIN") { implicit request =>
